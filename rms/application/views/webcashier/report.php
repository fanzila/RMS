	...
	<div data-role="content" data-theme="a">
		...
		<?php foreach ($lines as $m): ?>
			...
			<div id="<?=$m['mov']['id']?>" data-role="collapsible" style="background-color: <? if ($mov == 'close') { ?> 
				...
				<h2><a id="<?=$m['mov']['id']?>"></a>ID: <? $dateid = new DateTime($m['mov']['date']); echo date_format($dateid, 'Y-m-d'); echo " [".$m['mov']['id']."]";    ?> - <?=strtoupper($m['mov']['movement'])?><?if ($mov == 'close') { ?> - 
					<? if($m['mov']['status'] == 'validated') { ?>✅- <? } ?>
					<? if($m['mov']['status'] == 'error') { ?>❌- <? } ?>
					<? if($m['mov']['status'] == 'ok') { ?>👍- <? } ?>
					<?=$m['mov']['username']?><? } ?> <?if ($mov == 'close' && (count($m['cancelledReceipts']) > 0)) { echo " - <strong>" . count($m['cancelledReceipts']) ." receipts cancelled</strong>";}?></h2>
