...
class Auth extends CI_Controller
{
	...
	function create_user()
	{
		...
		$this->data['first_shift'] 	 = $first_shift;
		$this->data['txtmessage'] 	 = $this->input->post('txtmessage');
	...
	/**
	//check if a snapshift user has cost filed when shifted
	public function cliCheckStaffCost()
	{
		if (!is_cli()) return 1;

		$planning 	= $this->tools->snapplanning();
		SnapConnect("/api/v1/contracts?location_id=xxx")


		Vous pouvez retrouver les informations de salaire sur 
		l'endpoint : /api/v1/contracts?location_id=xxx sous la clé "hourly_gross_rate".
