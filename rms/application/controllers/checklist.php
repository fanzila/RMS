...
class Checklist extends CI_Controller {
	...
	*/
	...
	public function index()
	{
		...
		$checklist_req = "SELECT `name`, `id` FROM ckl_checklists WHERE active=1 ORDER BY `order` ASC";
	...
	{		
		...
		$checklist_rec_req = "SELECT r.user, u.first_name AS first_name, u.last_name AS last_name, r.id AS lid, r.id_checklist, r.date, c.name 
			FROM ckl_checklist_records AS r 
			JOIN ckl_checklists AS c ON c.id = r.id_checklist 
	...
	{		
		...
		if($load > 0) {
			$checklist_rec_req 	= "SELECT r.user, r.id AS rec_id, r.data, r.id_checklist, r.date, c.name FROM ckl_checklist_records AS r JOIN ckl_checklists AS c ON c.id = r.id_checklist WHERE r.id = $load";
			$checklist_rec_res	= $this->db->query($checklist_rec_req) or die($this->mysqli->error);
		...
		$checklist_req = "SELECT `name`, `id`  FROM ckl_checklists WHERE id = $id_ckl ORDER BY `order` ASC";
		$checklist_res = $this->db->query($checklist_req);
		$checklists = $checklist_res->row();

		if($id_ckl == 2 OR $id_ckl == 3 OR $id_ckl == 4) { 	
			$checklist_task_req = "SELECT `name`, `id`, `comment`, `priority`, `day_month_num`, `day_week_num` FROM ckl_checklist_tasks WHERE active=1 AND id_checklist = $id_ckl ORDER BY `order` ASC";
	...
	private function saveTasks() {
		...
		if($checklist_rec_id > 0) {
			$req = "UPDATE ckl_checklist_records SET `user` = '".$this->input->post('user')."', `date` = NOW(), `data` = '".$srl."' WHERE id = $checklist_rec_id";
			$email['subject'] = 'Checklist '.$checklist_name.' UPDATED';

		} else {
			$req = "INSERT INTO ckl_checklist_records SET `user` = '".$this->input->post('user')."', `date` = NOW(), `id_checklist` = ".$this->input->post('id_checklist').", `data` = '".$srl."'";	
			$email['subject'] = 'Checklist '.$checklist_name.' CREATED';
		}	
		...
		foreach ($this->input->post() as $key => $var) {
			...
			if($line[0] == 'comment' AND !empty($var)) {
				...
				$checklist_task_req = "SELECT `name` FROM ckl_checklist_tasks WHERE id= ".$line[1];
