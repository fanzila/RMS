...
class webCashier extends CI_Controller {
	...
	public function save()
	{
		...
		if($this->input->post('mov') == 'close') {
			...
			//error_log("$diff - $alert_amount[cashier_alert_amount_close_min] - $alert_amount[cashier_alert_amount_close_min]". $test_diff.$test_diff_control);
        ...
        $this->mmail->prepare($subject, $msg)
				...
				if($test_diff_control) $this->db->set('status', 'error');
