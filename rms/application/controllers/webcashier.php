...
class webCashier extends CI_Controller {
	...
	private function closing($file, $pmid)
	{
		...
		foreach ($d['ca'] as $key => $val) {
			...
			$rup = $this->db->get() or die('ERROR '.$this->db->_error_message().error_log('ERROR '.$this->db->_error_message()));
			...
			if(empty($af)) {
				...
				$this->db->insert('pos_payments');
				$rupi = $this->db->get() or die('ERROR '.$this->db->_error_message().error_log('ERROR '.$this->db->_error_message()));
