...
The temperature should be max: ".$max."° and min: ".$min."°";
						...
						$this->db->set('lastalarm', 'NOW()')->where('id_sensor', $val['id_sensor']);
						$this->db->update('sensors_alarm');
						$ru = $this->db->get() or die('ERROR '.$this->db->_error_message().error_log('ERROR '.$this->db->_error_message()));	
