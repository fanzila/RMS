...
class Product {
	...
	public function getProducts($id = null, $supplier_id = null, $order = null, $term = null, $id_bu, $active = null) {
		...
		$CI->db->select('p.id, s.id as supplier_id, p.supplier_reference, pc.name AS category_name, pc.color AS category_color, p.name AS name, s.name AS supplier_name, p.price, p.active, p.id_category, puprc.name AS unit_name, p.id_unit AS id_unit, p.daily_unit_conso AS daily_unit_conso, p.packaging AS packaging, p.freq_inventory, p.comment, ps.mini AS stock_mini, ps.max AS stock_max, ps.qtty AS stock_qtty, ps.warning AS stock_warning, ps.last_update_user AS last_update_user, ps.last_update_pos AS last_update_pos, u.username AS last_update_user_name, p.manage_stock')->from('products AS p')->join('suppliers AS s', 'p.id_supplier = s.id')->join('products_unit AS puprc', 'p.id_unit = puprc.id')->join('products_category AS pc','p.id_category = pc.id','left')->join('products_stock AS ps','p.id = ps.id_product','right')->join('users AS u','ps.last_update_id_user = u.id','left');
	...
	public function getProductsWithFilters($id= null, $order = null, $id_bu, $filters = null, $page = 1) 
	{
		...
		$CI->db->select('p.id, s.id as supplier_id, p.supplier_reference, pc.name AS category_name, p.name AS name, s.name AS supplier_name, p.price, p.active, p.id_category, puprc.name AS unit_name, p.id_unit AS id_unit, p.packaging AS packaging, p.freq_inventory, p.comment, p.daily_unit_conso AS daily_unit_conso, ps.mini AS stock_mini, ps.max AS stock_max, ps.qtty AS stock_qtty, ps.warning AS stock_warning, ps.last_update_user AS last_update_user, ps.last_update_pos AS last_update_pos, u.username AS last_update_user_name, p.manage_stock')->from('products AS p')->join('suppliers AS s', 'p.id_supplier = s.id')->join('products_unit AS puprc', 'p.id_unit = puprc.id')->join('products_category AS pc','p.id_category = pc.id','left')->join('products_stock AS ps','p.id = ps.id_product','right')->join('users AS u','ps.last_update_id_user = u.id','left');
	...
	public function countProductsWithFilters($id= null, $order = null, $id_bu, $filters = null) {
		...
		$CI->db->select('p.id, s.id as supplier_id, p.supplier_reference, pc.name AS category_name, p.name AS name, s.name AS supplier_name, p.price, p.active, p.id_category, puprc.name AS unit_name, p.id_unit AS id_unit, p.packaging AS packaging, p.daily_unit_conso AS daily_unit_conso, p.freq_inventory, p.comment, ps.mini AS stock_mini, ps.max AS stock_max, ps.qtty AS stock_qtty, ps.warning AS stock_warning, ps.last_update_user AS last_update_user, ps.last_update_pos AS last_update_pos, u.username AS last_update_user_name, p.manage_stock')->from('products AS p')->join('suppliers AS s', 'p.id_supplier = s.id')->join('products_unit AS puprc', 'p.id_unit = puprc.id')->join('products_category AS pc','p.id_category = pc.id','left')->join('products_stock AS ps','p.id = ps.id_product','right')->join('users AS u','ps.last_update_id_user = u.id','left');
		if (isset($filters) && !empty($filters)) {
			foreach ($filters as $key => $value) {
				if ($value != "" && $key != 'p.name' && $key != 'p.supplier_reference') {
					$CI->db->where($key, $value);
