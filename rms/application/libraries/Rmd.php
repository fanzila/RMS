...
class Rmd {
	...
	public function getAllTasks($id_bu)
	{
		...
		$CI->db->select('T.id as tid, T.task as ttask, T.comment as tcomment, T.active as tactive, T.priority as tpriority, T.notify_tablet as tnotify_tablet, T.type as ttype, N.start as nstart, N.end as nend, N.interval as ninterval, N.last as nlast, M.start as mstart, repeat_interval');
	...
	public function getTasks($task_id = null, $view = null, $id_bu)
	{
		...
		$CI->db->select('T.task as task, T.comment as comment, T.id as id, M.start as start, T.priority as priority, T.notify_tablet as notify_tablet, T.type as type, M.repeat_interval as `interval`, ROUND((UNIX_TIMESTAMP() - (UNIX_TIMESTAMP(start)+repeat_interval))/86400) as overdue')->from('rmd_tasks as T')->join('rmd_meta as M','M.id_task = T.id','right')->where('T.active', 1)->where('T.id_bu', $id_bu);
